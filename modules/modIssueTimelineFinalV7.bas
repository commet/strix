Attribute VB_Name = "modIssueTimelineFinalV7"
Option Explicit

' 최종 버전 V7 - 구체적이고 현실적인 이슈들
Private allIssues As Collection
Private filteredIssues As Collection

Sub CreateFinalDashboardV7()
    Dim ws As Worksheet
    Dim row As Long
    Dim btn As Object
    ...
    ' 데이터 로드
    Call LoadAllIssuesFinalV7
    
    ' 전체 이슈 표시
    Call ApplyFiltersV7(ws)
    
    ' 컬럼 너비 조정
    ws.Columns("A").ColumnWidth = 5   ' No
    ws.Columns("B").ColumnWidth = 12  ' 날짜
    ws.Columns("C").ColumnWidth = 75  ' 제목 (더 넓게)
    ws.Columns("D:E").ColumnWidth = 10  ' 분류
    ws.Columns("F").ColumnWidth = 15  ' 담당부서
    ws.Columns("G").ColumnWidth = 10  ' 상태
    ...
    MsgBox "Issue Timeline Dashboard가 생성되었습니다!" & vbCrLf & _
           "총 54개 이슈가 로드되었습니다.", vbInformation
End Sub

Private Sub LoadAllIssuesFinalV7()
    ...
    ' 사외 - LG에너지솔루션 RE+ 전시회
    Set issue = CreateIssueFinalV7(#8/29/2025#, "LG에너지솔루션, 9월 각형 배터리 공개 - RE+ 2025 전시회", _
                "사외", "경쟁사", "모니터링", "전략기획팀", _
                "분석보고서.pdf", #8/28/2025#, 85, #8/1/2025#, #11/30/2025#, True)
    allIssues.Add issue
    
    ' 사내 - BMW 차세대 모델 공급계약
    Set issue = CreateIssueFinalV7(#8/28/2025#, "BMW iX4 2026년형 46시리즈 원통형 배터리 20GWh 공급계약 협상", _
                "사내", "Marketing", "진행중", "영업마케팅팀", _
                "BMW_계약서_초안.docx", #8/27/2025#, 70, #7/1/2025#, #10/31/2025#, False)
    allIssues.Add issue
    
    ' 사외 - 삼성SDI 조직개편
    Set issue = CreateIssueFinalV7(#8/29/2025#, "삼성SDI 조직개편 - 극판센터 신설 및 전략마케팅 통합", _
                "사외", "경쟁사", "모니터링", "전략기획팀", _
                "경쟁사분석.pptx", #8/28/2025#, 90, #8/1/2025#, #11/30/2025#, False)
    allIssues.Add issue
    
    ' 사내 - 헝가리 3공장 증설
    Set issue = CreateIssueFinalV7(#8/27/2025#, "헝가리 3공장 NCM811 라인 월 15GWh 증설 프로젝트 착공", _
                "사내", "Production", "진행중", "생산관리팀", _
                "헝가리3공장_증설.xlsx", #8/26/2025#, 45, #6/1/2025#, #12/31/2025#, False)
    allIssues.Add issue
    
    ' 사외 - CATL 리튬 가격
    Set issue = CreateIssueFinalV7(#8/13/2025#, "CATL 리튬 광산 운영 중단으로 리튬 가격 8% 급등", _
                "사외", "시장", "미해결", "구매팀", _
                "원자재시장분석.pdf", #8/12/2025#, 25, #7/1/2025#, #10/31/2025#, False)
    allIssues.Add issue
    
    ' 사내 - 전고체 배터리 파일럿
    Set issue = CreateIssueFinalV7(#8/26/2025#, "전고체 배터리 파일럿 라인 월 200MWh 시험생산 목표 달성", _
                "사내", "R&D", "해결됨", "R&D센터", _
                "전고체_성과보고.docx", #8/25/2025#, 100, #3/1/2025#, #8/26/2025#, False)
    allIssues.Add issue
    
    ' 사외 - 두산밥캣 연구소
    Set issue = CreateIssueFinalV7(#8/27/2025#, "두산밥캣 eFORCE LAB 배터리팩 연구소 출범 - BSUP 개발", _
                "사외", "Tech", "모니터링", "R&D센터", _
                "기술동향.pdf", #8/26/2025#, 75, #8/1/2025#, #11/30/2025#, False)
    allIssues.Add issue
    
    ' 사내 - 메르세데스 벤츠 계약
    Set issue = CreateIssueFinalV7(#8/25/2025#, "메르세데스 벤츠 EQS 2027년형 NCM9 배터리 30GWh 독점공급 확정", _
                "사내", "Marketing", "해결됨", "영업마케팅팀", _
                "MB_계약완료.pdf", #8/24/2025#, 100, #5/1/2025#, #8/25/2025#, False)
    allIssues.Add issue
    
    ' 사외 - 중국 분리막
    Set issue = CreateIssueFinalV7(#8/24/2025#, "중국 8개 분리막 기업 향후 2년간 신규 증설 중단 합의", _
                "사외", "시장", "모니터링", "구매팀", _
                "공급망분석.xlsx", #8/23/2025#, 95, #8/1/2025#, #11/30/2025#, False)
    allIssues.Add issue
    
    ' 사내 - 원가절감 TF
    Set issue = CreateIssueFinalV7(#8/23/2025#, "2025년 하반기 원가 20% 절감 TF - 음극재 대체소재 개발", _
                "사내", "R&D", "진행중", "R&D센터", _
                "원가절감계획.pptx", #8/22/2025#, 60, #7/1/2025#, #11/30/2025#, False)
    allIssues.Add issue
    
    ' 사외 - SK온 에코프로
    Set issue = CreateIssueFinalV7(#8/22/2025#, "SK온-에코프로 폐배터리 재활용 협력 및 블랙파우더 공급계약", _
                "사외", "ESS", "모니터링", "ESS사업팀", _
                "ESG동향.pdf", #8/21/2025#, 85, #8/1/2025#, #11/30/2025#, True)
    allIssues.Add issue
    
    ' 사내 - 중국 창저우 공장
    Set issue = CreateIssueFinalV7(#8/21/2025#, "중국 창저우 2공장 LFP 배터리 월 10GWh 양산 승인", _
                "사내", "Production", "해결됨", "중국사업팀", _
                "창저우_양산승인.docx", #8/20/2025#, 100, #4/1/2025#, #8/21/2025#, False)
    allIssues.Add issue
    
    ' 사외 - SK온 조지아
    Set issue = CreateIssueFinalV7(#8/20/2025#, "SK온 조지아 공장 12개 라인 중 2개 ESS용 LFP 라인 배정", _
                "사외", "경쟁사", "모니터링", "전략기획팀", _
                "경쟁사분석.pdf", #8/19/2025#, 90, #8/1/2025#, #11/30/2025#, True)
    allIssues.Add issue
    
    ' 사내 - 현대차 가격협상
    Set issue = CreateIssueFinalV7(#8/19/2025#, "현대차 아이오닉6 2026년형 배터리 단가 5% 인하 요구 대응", _
                "사내", "Marketing", "미해결", "영업마케팅팀", _
                "현대차_협상안.xlsx", #8/18/2025#, 35, #7/15/2025#, #9/30/2025#, False)
    allIssues.Add issue
    
    ' 사외 - Subaru 전고체
    Set issue = CreateIssueFinalV7(#8/18/2025#, "Subaru 전고체 배터리 탑재 산업용 로봇 테스트 - Maxell PSB401010H", _
                "사외", "Tech", "모니터링", "R&D센터", _
                "기술벤치마킹.pdf", #8/17/2025#, 70, #8/1/2025#, #11/30/2025#, False)
    allIssues.Add issue
    
    ' 사내 - 폴란드 공장 화재
    Set issue = CreateIssueFinalV7(#8/15/2025#, "폴란드 1공장 3번 라인 화재사고 - 생산 차질 복구 계획", _
                "사내", "Production", "미해결", "안전환경팀", _
                "사고보고서.docx", #8/14/2025#, 20, #8/15/2025#, #10/15/2025#, False)
    allIssues.Add issue
    
    ' 사외 - BMW 중국산
    Set issue = CreateIssueFinalV7(#8/14/2025#, "BMW 미국에서 중국산 배터리 도입 검토 - 82% 관세에도 경제성 판단", _
                "사외", "정책", "모니터링", "영업마케팅팀", _
                "시장분석.pdf", #8/13/2025#, 85, #8/1/2025#, #11/30/2025#, False)
    allIssues.Add issue
    
    ' 사내 - Validation 단축
    Set issue = CreateIssueFinalV7(#8/12/2025#, "GM Ultium 플랫폼 Validation 기간 6개월→3개월 단축 협의 완료", _
                "사내", "R&D", "해결됨", "R&D센터", _
                "GM_협의결과.pdf", #8/11/2025#, 100, #5/1/2025#, #8/12/2025#, False)
    allIssues.Add issue
    
    ' 사내 - 성과평가 제도
    Set issue = CreateIssueFinalV7(#8/10/2025#, "사내 HR 주요 변경사항 업데이트 - 성과평가 제도 개편", _
                "사내", "Staff", "해결됨", "인사팀", _
    ...
    ' 사외 - SK온 SK엔무브
    Set issue = CreateIssueFinalV7(#7/31/2025#, "SK온-SK엔무브 합병 - 11월 1일 통합법인 출범", _
                "사외", "경쟁사", "진행중", "전략기획팀", _
                "M&A분석.pdf", #7/30/2025#, 70, #6/1/2025#, #11/1/2025#, False)
    allIssues.Add issue
    
    ' 사내 - 이사회
    Set issue = CreateIssueFinalV7(#7/30/2025#, "3분기 실적 영업이익 3.2조원 달성 - 이사회 4분기 투자 15조원 승인", _
                "사내", "Staff", "해결됨", "경영기획팀", _
                "이사회자료.pptx", #7/29/2025#, 100, #7/1/2025#, #7/30/2025#, False)
    allIssues.Add issue
    
    ' 사외 - CATL 관광선
    Set issue = CreateIssueFinalV7(#7/29/2025#, "CATL 중국 첫 전기 관광선 '위젠 77' 선박용 배터리 공급", _
                "사외", "경쟁사", "모니터링", "시장분석팀", _
                "신시장분석.pdf", #7/28/2025#, 85, #7/1/2025#, #11/30/2025#, False)
    allIssues.Add issue
    
    ' 사내 - 테슬라 협상
    Set issue = CreateIssueFinalV7(#7/28/2025#, "테슬라 4680 배터리 연간 50GWh 공급계약 최종 협상 진행", _
                "사내", "Marketing", "진행중", "영업마케팅팀", _
                "테슬라_계약서.docx", #7/27/2025#, 85, #5/1/2025#, #9/30/2025#, False)
    allIssues.Add issue
    
    ' 사내 - 합병 시너지
    Set issue = CreateIssueFinalV7(#7/25/2025#, "계열사 합병 시너지 3조원 도출 - 중복 R&D 통합 및 구매력 강화", _
                "사내", "Staff", "진행중", "경영기획팀", _
                "PMI보고서.xlsx", #7/24/2025#, 65, #5/15/2025#, #10/31/2025#, False)
    allIssues.Add issue
    
    ' 사외 - LG엔솔 특허
    Set issue = CreateIssueFinalV7(#7/24/2025#, "LG엔솔 Sunwoda 특허침해 소송 승소 - 독일 판매금지", _
                "사외", "특허", "해결됨", "법무팀", _
                "특허분석.pdf", #7/23/2025#, 100, #5/1/2025#, #7/24/2025#, False)
    allIssues.Add issue
    
    ' 사내 - 인도 공장
    Set issue = CreateIssueFinalV7(#7/22/2025#, "인도 첸나이 배터리 공장 2026년 상반기 착공 최종 승인", _
                "사내", "투자", "해결됨", "해외사업팀", _
                "인도투자안.pptx", #7/21/2025#, 100, #3/1/2025#, #7/22/2025#, False)
    allIssues.Add issue
    
    ' 사내 - 공장 화재
    Set issue = CreateIssueFinalV7(#7/20/2025#, "울산 2공장 건식코팅 라인 화재 - 월 2GWh 생산차질 발생", _
                "사내", "Production", "미해결", "안전환경팀", _
                "사고보고서.pdf", #7/19/2025#, 30, #7/15/2025#, #9/30/2025#, False)
    allIssues.Add issue
    
    ' 사외 - 현대차 SK온
    Set issue = CreateIssueFinalV7(#7/18/2025#, "현대차-SK온 아이오닉5 액침냉각 배터리 선행개발 시험", _
                "사외", "Tech", "진행중", "R&D센터", _
                "기술동향.pdf", #7/17/2025#, 70, #6/1/2025#, #11/30/2025#, False)
    allIssues.Add issue
    
    ' 사외 - LG 미국 ESS
    Set issue = CreateIssueFinalV7(#7/18/2025#, "LG에너지솔루션, 미국 뉴멕시코주 600MWh급 ESS 수주", _
                "사외", "ESS", "모니터링", "ESS사업팀", _
                "ESS시장분석.pdf", #7/17/2025#, 95, #7/1/2025#, #11/30/2025#, True)
    allIssues.Add issue
    
    ' 사외 - 독일 전기차
    Set issue = CreateIssueFinalV7(#7/17/2025#, "2025 상반기 독일 전기차 판매량 35% 증가 - 248,726대", _
                "사외", "시장", "모니터링", "해외사업팀", _
                "시장보고서.xlsx", #7/16/2025#, 95, #7/1/2025#, #11/30/2025#, False)
    allIssues.Add issue
    
    ' 사외 - 기아 CATL
    Set issue = CreateIssueFinalV7(#7/17/2025#, "기아 EV5 내수 모델에 CATL 삼원계 배터리 탑재 방침", _
                "사외", "경쟁사", "모니터링", "전략기획팀", _
                "경쟁사동향.pdf", #7/16/2025#, 90, #7/1/2025#, #11/30/2025#, False)
    allIssues.Add issue
    
    ' 사내 - 리튬 공동구매
    Set issue = CreateIssueFinalV7(#7/15/2025#, "현대기아차 리튬 연간 20만톤 공동구매 MOU 체결", _
                "사내", "Marketing", "해결됨", "구매팀", _
                "MOU계약서.pdf", #7/14/2025#, 100, #5/1/2025#, #7/15/2025#, False)
    allIssues.Add issue
    
    ' 사내 - 중국 미팅
    Set issue = CreateIssueFinalV7(#7/10/2025#, "중국 공신부 차관 미팅 - 상하이 3공장 증설 허가 획득", _
                "사내", "Staff", "해결됨", "중국사업팀", _
                "미팅결과.docx", #7/9/2025#, 100, #5/1/2025#, #7/10/2025#, False)
    allIssues.Add issue
    
    ' 사외 - Gotion 독일
    Set issue = CreateIssueFinalV7(#7/8/2025#, "Gotion High-Tech, 독일 괴팅겐 5MWh 액체냉각 ESS 현지생산", _
                "사외", "ESS", "모니터링", "ESS사업팀", _
    ...
    ' 사외 - EVE Energy
    Set issue = CreateIssueFinalV7(#6/30/2025#, "EVE Energy 말레이시아 ESS 배터리 공장 86.5억위안 투자", _
                "사외", "투자", "모니터링", "전략기획팀", _
                "투자분석.pdf", #6/29/2025#, 90, #6/1/2025#, #11/30/2025#, True)
    allIssues.Add issue
    
    ' 사내 - 포드 계약
    Set issue = CreateIssueFinalV7(#6/28/2025#, "포드 F-150 Lightning 2026년형 NCM622 배터리 40GWh 계약 체결", _
                "사내", "Marketing", "해결됨", "영업마케팅팀", _
                "포드_계약완료.pdf", #6/27/2025#, 100, #4/1/2025#, #6/28/2025#, False)
    allIssues.Add issue
    
    ' 사외 - BYD 전기버스
    Set issue = CreateIssueFinalV7(#6/24/2025#, "BYD, 유럽에서 주행거리 700km 전기버스 eBus B13.b 공개", _
                "사외", "경쟁사", "모니터링", "시장분석팀", _
                "BYD분석.pptx", #6/23/2025#, 85, #6/1/2025#, #11/30/2025#, False)
    allIssues.Add issue
    
    ' 사내 - 실리콘 음극재
    Set issue = CreateIssueFinalV7(#6/20/2025#, "실리콘 음극재 5% 적용 NCM9 배터리 에너지밀도 320Wh/kg 달성", _
                "사내", "R&D", "해결됨", "R&D센터", _
                "연구성과.docx", #6/19/2025#, 100, #2/1/2025#, #6/20/2025#, False)
    allIssues.Add issue
    
    ' 사외 - 삼성SDI 전고체
    Set issue = CreateIssueFinalV7(#6/9/2025#, "삼성SDI 전고체 배터리 롤프레스 장비 PO 발주", _
                "사외", "Tech", "모니터링", "R&D센터", _
    ...
    ' 사외 - Gotion 전고체
    Set issue = CreateIssueFinalV7(#5/19/2025#, "Gotion 전고체 배터리 300Wh/kg 전기차 1000km 시험주행", _
                "사외", "Tech", "모니터링", "R&D센터", _
                "기술벤치마킹.pdf", #5/18/2025#, 95, #5/1/2025#, #11/30/2025#, False)
    allIssues.Add issue
    
    ' 사내 - 멕시코 공장
    Set issue = CreateIssueFinalV7(#5/17/2025#, "멕시코 몬테레이 신규 공장 부지 250만㎡ 매입 완료", _
                "사내", "투자", "해결됨", "해외사업팀", _
                "멕시코투자.xlsx", #5/16/2025#, 100, #3/1/2025#, #5/17/2025#, False)
    allIssues.Add issue
    
    ' 사외 - LG 폴란드
    Set issue = CreateIssueFinalV7(#5/15/2025#, "LG엔솔 폴란드 브로츠와프 ESS 전용 라인 연말 가동", _
                "사외", "ESS", "모니터링", "해외사업팀", _
                "경쟁사동향.pdf", #5/14/2025#, 100, #5/1/2025#, #11/30/2025#, True)
    allIssues.Add issue
    
    ' 사외 - 태국 Sunwoda
    Set issue = CreateIssueFinalV7(#5/14/2025#, "태국 정부, Sunwoda의 10억 달러 배터리 공장 투자 승인", _
                "사외", "투자", "모니터링", "시장분석팀", _
                "동남아시장.pdf", #5/13/2025#, 90, #5/1/2025#, #11/30/2025#, False)
    allIssues.Add issue
    
    ' 사내 - LFP 양산
    Set issue = CreateIssueFinalV7(#5/10/2025#, "국내 최초 LFP 배터리 양산라인 월 5GWh 가동 개시", _
                "사내", "Production", "해결됨", "생산관리팀", _
                "LFP양산보고.pptx", #5/9/2025#, 100, #1/1/2025#, #5/10/2025#, False)
    allIssues.Add issue
    
    ' 사외 - CATL ESS
    Set issue = CreateIssueFinalV7(#5/9/2025#, "CATL 9MWh 초대형 ESS 'Tener Stack' 공개 - EES 유럽 2025", _
                "사외", "ESS", "모니터링", "ESS사업팀", _
                "제품분석.pdf", #5/8/2025#, 100, #5/1/2025#, #11/30/2025#, True)
    allIssues.Add issue
    
    ' 사내 - 리사이클링 센터
    Set issue = CreateIssueFinalV7(#5/5/2025#, "충북 청주 배터리 리사이클링 센터 연 10만톤 처리시설 착공", _
                "사내", "투자", "진행중", "ESS사업팀", _
                "리사이클링계획.docx", #5/4/2025#, 40, #4/1/2025#, #12/31/2025#, False)
    allIssues.Add issue
End Sub

Private Function CreateIssueFinalV7(issueDate As Date, title As String, category1 As String, _
    ...
    Set CreateIssueFinalV7 = issue
End Function

Sub ApplyFiltersV7(ws As Worksheet)
    ...
    ' 필터링된 이슈 표시
    Call DisplayFilteredIssuesV7(ws)
End Sub

' 워크시트에서 직접 호출할 수 있는 Public 서브루틴
Public Sub FilterChanged()
    On Error Resume Next
    Call ApplyFiltersV7(ThisWorkbook.Worksheets("Issue Timeline"))
    On Error GoTo 0
End Sub

Sub DisplayFilteredIssuesV7(ws As Worksheet)
    Dim row As Long
    Dim issue As Object
    ...
    For Each issue In filteredIssues
        ...
        Call AddFinalIssueRowV7(ws, row, displayCount, issue)
        row = row + 1
    Next issue
...
Private Sub AddFinalIssueRowV7(ws As Worksheet, row As Long, no As Integer, issue As Object)
    ' 번호
    ...
    ' 담당부서
    ws.Cells(row, 6).Value = issue("dept")
    ws.Cells(row, 6).HorizontalAlignment = xlCenter
    
    ' 상태 (배경색 없이, 텍스트 색상만)
    ws.Cells(row, 7).Value = issue("status")
    ws.Cells(row, 7).HorizontalAlignment = xlCenter
    ws.Cells(row, 7).Font.Bold = True
    Select Case issue("status")
        Case "해결됨"
            ws.Cells(row, 7).Font.Color = RGB(0, 176, 80)
        Case "진행중"
            ws.Cells(row, 7).Font.Color = RGB(255, 192, 0)
        Case "미해결"
            ws.Cells(row, 7).Font.Color = RGB(255, 0, 0)
        Case "모니터링"
            ws.Cells(row, 7).Font.Color = RGB(0, 112, 192)
    End Select
    ...
    ' 타임라인 그리기
    Call DrawFinalTimelineV7(ws, row, issue)
End Sub

Private Sub DrawFinalTimelineV7(ws As Worksheet, row As Long, issue As Object)
    ...
    If startCol <= 15 Then
        ...
        If startCol >= 9 And startCol <= 15 Then
            Call AddFinalMarkerV7(ws, row, startCol, "circle")
        End If
        ...
        If issue("status") = "해결됨" Then
            ...
            If endCol >= 9 And endCol <= 15 Then
                Call AddFinalMarkerV7(ws, row, endCol, "check")
            End If
        ElseIf currentCol = 12 Then  ' 8월 위치
            ' 현재 위치에 상태 마커
            Select Case issue("status")
                Case "진행중"
                    Call AddFinalMarkerV7(ws, row, 12, "arrow")
                Case "모니터링"
                    Call AddFinalMarkerV7(ws, row, 12, "arrow")
                Case "미해결"
                    Call AddFinalMarkerV7(ws, row, 12, "square")
            End Select
        End If
    End If
End Sub
...
Private Sub AddFinalMarkerV7(ws As Worksheet, row As Long, col As Integer, markerType As String)
    Dim cell As Range
